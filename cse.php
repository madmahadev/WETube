...
<head>
    ...
    <script type="text/javascript">
        ...
        function suggest()
        {
            ...
            xmlhttp.open('GET','search.php?query='+document.searchform.searchquery.value,'true');
            xmlhttp.send();
        }
    ...
    if(@$connect=mysql_connect('localhost','root',''))
    {
        if(@mysql_select_db('wetube'))
        {
            ...
            while(@$query_row=mysql_fetch_assoc($query_run))
            {
                ...
                echo('
              <div class="subjects">
                <a href="cse.php?link_name='.$query_row['subject'].'">->'.$query_row['subject'].'</a><br>
               </div>
               ');
            }
       ...
       $sub_name="";
        ...
        if(isset($_GET['link_name']))
        {
            if(!empty($_GET['link_name'])) {
                ...
                $query = 'select video_name from cse_' . $sub_name;
                $query_run = mysql_query($query);

                while (@$query_row = mysql_fetch_assoc($query_run)) {
                    $name = $query_row['video_name'];
                    $link = 'videos/cse/' . $sub_name . '/' . $name . '&link_name=' . $sub_name;

                    echo('
                    <div class="video">

                <a href="cse.php?video_name=' . $link . '">' . $name . '</a><br>
                </div>
               ');
                }
            }
...
<div class="search">
    <form name="searchform" action="<?php echo'cse.php'?>" method="get">
...
?>
   ...
   <br><br> Discussion Board ...comming soon.
</div  >
