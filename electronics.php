...
<div id="subject">
    ...
    if(@$connect=mysql_connect('localhost','root',''))
    {
        ...
        {
            $query='select subject from electronics_sub';
            ...
            while(@$query_row=mysql_fetch_assoc($query_run))
            {
                ...
                echo('
              <div class="subjects">
                <a href="electronics.php?link_name='.$query_row['subject'].'">->'.$query_row['subject'].'</a><br>
               </div>
               ');
            }
    ...
    <div class="videolink">
        ...
        if(isset($_GET['link_name']))
        {
            if(!empty($_GET['link_name'])) {
                ...
                $query = 'select video_name from electronics_' . $sub_name;
                $query_run = mysql_query($query);

                while (@$query_row = mysql_fetch_assoc($query_run)) {
                    $name = $query_row['video_name'];
                    $link = 'videos/electronics/' . $sub_name . '/' . $name . '&link_name=' . $sub_name;

                    echo('
                    <div class="video">

                <a href="electronics.php?video_name=' . $link . '">' . $name . '</a><br>
                </div>
               ');
                }
            }
...
<div class="search">
    <form name="searchform" action="<?php echo'electronics.php'?>" method="get">
